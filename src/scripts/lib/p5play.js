...
p5.prototype.registerMethod("init", function p5PlayInit() {
	...
	const fixRound = (val) =>
		Math.abs(val - Math.round(val)) <= pl.Settings.linearSlop
			? Math.round(val)
			: val
	...
	this.Sprite = class {
		...
		constructor(x, y, w, h, collider) {
			...
			if (
				...
				(typeof args[0] == "string" ||
					args[0] instanceof this.p.SpriteAnimation ||
			...
			Object.defineProperties(this._vel, {
					...
					set(val) {
						...
						if (_this.body) {
							_this.body.setLinearVelocity(
								new pl.Vec2(val, _this.body.getLinearVelocity().y)
							)
						} else {
				...
				y: {
					...
					set(val) {
						...
						if (_this.body) {
							_this.body.setLinearVelocity(
								new pl.Vec2(_this.body.getLinearVelocity().x, val)
							)
						} else {
			...
			for (let i = 0; i < this.groups.length; i++) {
				...
				for (let prop of props) {
					if (
						...
						this.p.Sprite.propsAll.includes(prop)
					) {
						continue
					}
					let val = g[prop]
			...
			this.textSize ??=
				this.tileSize == 1 ? (this.p.canvas ? this.p.textSize() : 12) : 0.8
		...
		_parseColor(val) {
			...
			if (val._r !== undefined)
				return this.p.color(val._r, val._g, val._b, val._a * 255)
			if (val._h !== undefined)
				return this.p.color(val._h, val._s, val._v, val._a * 255)
		...
		_getVertices(output2DArrays) {
			...
			for (let i = 0; i < v.length; i++) {
				let arr = [
					fixRound((v[i].x / this.tileSize) * plScale + x),
					fixRound((v[i].y / this.tileSize) * plScale + y),
				]
		...
		set pos(val) {
			if (this.body) {
				let pos = new pl.Vec2(
					(val.x * this.tileSize) / plScale,
					(val.y * this.tileSize) / plScale
				)
		...
		setSpeed(speed, direction) {
			console.warn(
				"setSpeed is deprecated. Set sprite.direction and sprite.speed separately instead."
			)
		...
		moveTo(x, y, speed) {
			...
			let velThresh =
				Math.max(this.p.world.velocityThreshold, margin * 0.25) / this.tileSize
		...
		_ensureCollide(target, callback) {
			...
			if (
				!(target instanceof this.p.Sprite) &&
				!(target instanceof this.p.Group)
			) {
	...
	this.SpriteAnimation = class extends Array {
		...
		constructor() {
			...
			// SpriteSheet mode
				...
				!(args[args.length - 1] instanceof p5.Image)
			) {
		...
		update() {
			...
			if (this.playing && this._cycles % this.frameDelay === 0) {
				...
				if (
					(this.targetFrame == -1 && this.frame == this.lastFrame) ||
	...
	this.Group = class extends Array {
		 ...
		 */
		orbit(amount) {
			if (this.p.frameCount == 0)
				console.warn(
					"group.orbit is experimental and is subject to change in the future!"
				)
		...
		push(...sprites) {
			for (let s of sprites) {
				if (!(s instanceof this.p.Sprite)) {
					throw new Error(
						"you can only add sprites to a group, no " + typeof s + "s"
					)
				}
	...
	this.World = class extends pl.World {
		...
		step(timeStep, velocityIterations, positionIterations) {
			...
			super.step(
				timeStep || 1 / (this.p._targetFrameRate || 60),
		...
		getSpritesAt(x, y, group, cameraActiveWhenDrawn) {
			...
			aabb.lowerBound = new pl.Vec2(
				convertedPoint.x - 0.001,
				convertedPoint.y - 0.001
			)
			aabb.upperBound = new pl.Vec2(
				convertedPoint.x + 0.001,
				convertedPoint.y + 0.001
			)
			...
			this.queryAABB(aabb, (fxt) => {
				if (
					fxt.getShape().testPoint(fxt.getBody().getTransform(), convertedPoint)
				) {
					fxts.push(fxt)
				}
				return true
	...
	pl.Fixture.prototype.shouldCollide = function (that) {
		...
		if (!shouldOverlap)
			shouldOverlap = a.p.world._findContactCB("_hasOverlap", b, a)
	...
	this.updateSprites = function () {
		if (this.frameCount == 1)
			console.warn("updateSprites() is deprecated, use world.step() instead.")
	...
	this.drawSprite = function (sprite) {
		if (this.frameCount == 1)
			console.warn("drawSprite() is deprecated, use sprite.draw() instead.")
		sprite.draw()
	}
	...
	this.drawSprites = function (group) {
		if (this.frameCount == 1)
			console.warn("drawSprites() is deprecated, use group.draw() instead.")
	...
	this.play = (sound) => {
		if (!sound.play)
			throw new Error(
				"Tried to play a sound but the sound is not a sound object: " + sound
			)
...
canvas {
	...
	this.background = function () {
		...
		if (
			args.length == 1 &&
			(typeof args[0] == "string" || args[0] instanceof p5.Color)
		) {
	...
	errMsgs.Sprite.rotateTo[0] = errMsgs.Sprite.rotateTowards[0] =
	...
	class FriendlyError extends Error {
		constructor(func, errorNum, e) {
			...
			if (m.base) msg = m.base + ln
				...
				msg =
					errMsgs.generic[Math.floor(Math.random() * errMsgs.generic.length)] + ln
...
p5.prototype.registerMethod("post", function p5playPostDraw() {
	...
	if (this.world.mouseTracking) {
		...
		let uiSprites = this.world.getSpritesAt(
			this.camera.mouse.x,
			this.camera.mouse.y,
			this.allSprites,
			false
		...
		let ms
		if (
			this.mouse.pressing("left") ||
			this.mouse.pressing("center") ||
			this.mouse.pressing("right")
		) {
		...
		for (let s of this.world.mouseSprites) {
			if (
				(!this.world.mouseSprite?.mouse.draggable || s != ms) &&
				!sprites.includes(s)
			) {
				s.mouse.hover = -1
				s.mouse.left = 0
	...
	if (this.p5play._fps) {
		...
		this.p5play._fps =
			Math.round(
				1000 / (this.p5play._postDrawFrameTime - this.p5play._preDrawFrameTime)
			) || 1
	}
